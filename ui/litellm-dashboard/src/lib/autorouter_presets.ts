...
// The catalog is served by the proxy at /public/autorouter_presets (validated there against the
// same envelope), so a catalog update propagates without a dashboard release. There is
// deliberately no bundled copy here; see useAutoRouterPresets for the fetch.
