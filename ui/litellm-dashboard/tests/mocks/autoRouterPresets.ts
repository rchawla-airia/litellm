...
/**
 * Stubs the proxy-served preset catalog for any test that renders the auto-router tree, derived
 * from the real bundled catalog so a preset edit there flows into test expectations instead of
 * redding on a stale copy. Exported as a vi.fn so a test can override the query state.
 */
